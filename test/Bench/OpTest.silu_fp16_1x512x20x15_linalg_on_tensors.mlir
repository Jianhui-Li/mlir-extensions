...
module attributes {torch.debug_module_name = "SiLU"} {
  func.func @forward(%arg0: tensor<1x512x20x15xf16>) -> tensor<1x512x20x15xf16> {
    ...
    %2 = linalg.generic {indexing_maps = [#map, #map, #map1], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%1, %arg0 : tensor<1x512x20x15xf16>, tensor<1x512x20x15xf16>) outs(%0 : tensor<1x512x20x15xf16>) {
    ^bb0(%in: f16, %in_0: f16, %out: f16):
      %3 = arith.mulf %in, %in_0 : f16
      linalg.yield %3 : f16
    } -> tensor<1x512x20x15xf16>
    return %2 : tensor<1x512x20x15xf16>
  }
  func.func @main() {
    %0= arith.constant dense<1.3>:tensor<1x512x20x15xf16>
    %1 = call @forward(%0) : (tensor<1x512x20x15xf16>) -> tensor<1x512x20x15xf16>
    return
  }
}
